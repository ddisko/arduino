#include <Servo.h> //include servo library
 ...
 int filamentPin = 11; //pin our LED filament is connected to
 int meterPin = 5; //pin meter is connected to
 ...
 int ledState = LOW; //used to control LED state, we're starting with it OFF
 ...
 const unsigned long fadePeriod = 15; //interval to fade blue LED in milliseconds
...
// }
 ...
 void fadeLED() {
  ...
  if (currentMillis - fadePreviousMillis >= fadePeriod) {
   ...
   analogWrite(meterPin, brightness);
   analogWrite(filamentPin, brightness);
...
// }
 ...
 void setup() {
  ...
  pinMode(meterPin, OUTPUT); //set blue LED pin as an output
